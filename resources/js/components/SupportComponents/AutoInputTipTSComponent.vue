<template>
  <div class="impala-autoinput">
    ...
    <input type="text" class="impala-input" ref="auto_input" v-model="MainVarInInput" @blur="focus_out_from_select"
    ...
    <div class="impala-box:dropdown" v-if="showList">
      <ul class="select_list_gruzoot" ref="scrollContainer">
        <li v-for="(item, index) in filteredList" :key="index">
          <div class="impala-text-link:normal" @click="select(item)">{{ item.nazvanie }}</div>
        </li>
      </ul>
    </div>
  </div>
