<template>
  ...
  <modal-pogruzka-component
      ...
      vid="GradeComponent"
      ref="modalComponentforActionAuthor"
  ></modal-pogruzka-component>
  ...
  <input hidden="hidden" type="file" ref="filesmult" multiple v-on:change="handleMultFilesUpload()"/>
      ...
      <div class="impala-col-input">
        ...
        <div>
          <div class="impala-add-link:none"
               v-html="(data_vneseniya === '' ||  data_vneseniya === null) ? '---' : data_vneseniya"></div>
        </div>
      </div>
    </div>
  </div>
  <div class="impala-request__wrap p-5 justify-between items-start gap-6 flex">
    <aside class="w-[475px] m-0 flex-col justify-start items-start gap-6 flex grow-0 shrink-0">
      ...
      <div class="flex-col justify-center items-start gap-4 flex w-full">
        ...
        <div class="impala-box impala-box:link w-full p-4 flex-row justify-between items-center gap-6 flex"
             v-for="(elem, key) in spisokTSarr" v-on:click="add_to_right_column(key)">
          ...
          <div class="flex gap-2.5 items-center impala-text-gray-500">
            <div class="flex min-w-[45px]" v-html="((elem.kol_TS_TS) ? elem.kol_TS_TS : '--') + ' ТС'"></div>
            <div class="flex min-w-[95px]" v-html="((elem.stavka_TS) ? elem.stavka_TS : '-------') + ' ₽'"></div>
          </div>
      ...
      <div class="flex-col justify-center items-start gap-4 flex w-full">
        ...
        <div class="impala-box w-full p-4 flex-col justify-start items-start gap-6 flex">
          <div class="impala-row-input self-stretch" v-show="checkRolePermission([1])">
            ...
            <div class="impala-add-link:none"
                 v-html="(kompaniya_zakazchik === '' ||  kompaniya_zakazchik === null) ? '---' : kompaniya_zakazchik"></div>
          </div>
          <div class="impala-row-input self-stretch" v-show="checkRolePermission([1])">
            <div class="impala-label">
              Менеджер заказчика
            ...
            <div class="impala-add-link:none"
                 v-html="(menedzer_zakazchik === '' ||  menedzer_zakazchik === null) ? '---' : menedzer_zakazchik"></div>
          </div>
          <div class="impala-row-input" v-show="checkRolePermission([1])">
            ...
            <div class="impala-add-link:none"
                 v-html="(cena_kontrakta === '' ||  cena_kontrakta === null) ? '---' : cena_kontrakta"></div>
          </div>
        </div>
      </div>
    </aside>
    <div class="flex-col justify-center items-start gap-4 flex grow">
      ...
      <div v-for="(elem, key) in spisokTShead"
           class="impala-box self-stretch flex-col justify-center items-start gap-0 flex"
           :class="elem.id_ts===right_current_TS && 'impala-outline'">
        <div class="self-stretch p-4 justify-between items-center gap-4 inline-flex">
          ...
          <div class="flex-row justify-between items-start gap-6 inline-flex grow">
            <div class="impala-row-input self-stretch !gap-1.5 min-w-[175px]">
              <div class="impala-add-link:none !leading-none"
                   v-html="(elem.vid_TSNazvanie === '' ||  elem.vid_TSNazvanie === null) ? '---' : elem.vid_TSNazvanie"></div>
            ...
            <div class="impala-row-input self-stretch !gap-1.5 grow min-w-[375px]">
              <div class="impala-add-link:none !leading-none"
                   v-html="(elem.perevozchik_TSNazvanie) ? elem.perevozchik_TSNazvanie :  '---'"></div>
              <div class="impala-label !leading-none">
                перевозчик
              </div>
            </div>
            <div class="impala-row-input self-stretch !gap-1.5 min-w-[135px]">
              <div class="impala-add-link:none !leading-none"
                   v-html="(elem.adres_pogruzki_TS && elem.adres_pogruzki_TS[0] && elem.adres_pogruzki_TS[0].date_ts) ? elem.adres_pogruzki_TS[0].date_ts :  '---'"></div>
              <div class="impala-label !leading-none">
                погрузка
              </div>
            </div>
            <div class="impala-row-input self-stretch !gap-1.5 min-w-[135px]">
              <div class="impala-add-link:none !leading-none"
                   v-html="(elem.adres_vygr_TS && elem.adres_vygr_TS.slice(-1)[0] && elem.adres_vygr_TS.slice(-1)[0].date_ts) ? elem.adres_vygr_TS.slice(-1)[0].date_ts :  '---'"></div>
              <div class="impala-label !leading-none">
                выгрузка
              </div>
            </div>
            <div class="impala-row-input self-stretch !gap-1.5  min-w-[125px]">
              <div class="impala-add-link:none !leading-none"
                   v-html="((elem.stavka_summa) ? elem.stavka_summa :  '---') + ' ₽'"></div>
              <div class="impala-label !leading-none">
                ставка
              </div>
            </div>
          ...
          <div class="grow shrink basis-0 justify-end items-center gap-4 flex">
            <div v-on:click="click_on_right_col(elem,key)" class="impala-btn impala-btn-small impala-btn-gray"
                 v-html="elem.id_ts===right_current_TS ? 'Свернуть' : 'Подробнее'"></div>
          </div>
        </div>
        <div v-if="elem.id_ts===right_current_TS"
             class="self-stretch p-4 border-t border-[#e3eaea] justify-between items-start gap-4 inline-flex">
          <div class="grow shrink basis-0 flex-col justify-start items-start gap-6 inline-flex">
            <div class="grow shrink basis-0 flex-row justify-between items-end gap-6 inline-flex">
              <div class="self-stretch justify-between items-center inline-flex gap-4 grow">
                ...
                <div class="impala-row-input">
                  ...
                  <input class="impala-input" v-model="elem.kol_gruz_TS"
                         @input="update_one_data_number(elem,'kol_gruz_TS', elem.kol_gruz_TS)"/>
                </div>
                <div class="impala-row-input">
                  <div class="impala-label">Расстояние, км</div>
                  <input class="impala-input" v-model="elem.rasstojanie_TS"
                         @input="update_one_data_number(elem,'rasstojanie_TS', elem.rasstojanie_TS)"/>
                </div>
              </div>
              <div class="self-stretch justify-end items-end inline-flex gap-4 w-6/12">
                ...
                <div><input hidden="hidden" type="file" id="files_doc" ref="files_doc"
                  ...
                  <VMenu>
                    ...
                    <template #popper>
                      <div class="p-2 flex-col justify-center items-start gap-2.5 inline-flex">
                        ...
                        <div class="impala-text-link "
                             v-if="elem.perevozchik_TSNazvanie!=='' || elem.perevozchik_TSNazvanie!==null"
            ...
            <div class="grid grid-cols-2 gap-6 w-full">
              <div class="self-stretch flex-col justify-start items-start gap-4 flex">
                <template v-if="elem.adres_pogruzki_TS.length > 0">
                  <div v-for="(adres_pogr, key1) in elem.adres_pogruzki_TS" :key="adres_pogr.id_pogruzka"
                       class="self-stretch flex flex-col gap-4 pl-4 border-l-4 border-[#fff4de]">
                      ...
                      <div class="impala-label:base">
                        ...
                        <VMenu>
                          ...
                          <template #popper>
                            <div class="p-2 flex-col justify-center items-start gap-2.5 inline-flex">
                              <div class="impala-text-link "
                                   v-on:click="select_temp_var(key,'pogruzka',key1,elem,adres_pogr.id_pogruzka,1,adres_pogr.adres_pogruzki,'adres_pogruzki')">
                                <i
                    ...
                    <div class="impala-row-input">
                      ...
                      <date-picker type="datetime" valueType="format" v-model:value="adres_pogr.date_ts"
                                   ...
                                   @change="update_one_data_pogruzka(elem,adres_pogr.id_pogruzka,1,adres_pogr.date_ts,'date_ts')"
                    ...
                    <div class="impala-row-input">
                      ...
                      <div v-if="adres_pogr.doc_name_FOTO.length > 0"
                    ...
                    <div class="impala-row-input">
                      ...
                      <div v-if="adres_pogr.doc_name_DOC.length > 0"
                           class="impala-box:dashed flex flex-col gap-2 self-stretch p-2">
              ...
              <div class="self-stretch flex-col justify-start items-start gap-4 flex">
                <template v-if="elem.adres_vygr_TS.length > 0">
                  <div v-for="(adres_vygr, key2) in elem.adres_vygr_TS" :key="adres_vygr.id_pogruzka"
                       class="self-stretch flex flex-col gap-4 pl-4 border-l-4 border-[#c9f7f5]">
                    <div class="impala-row-input w-full">
                      <div class="impala-label:base">
                        ...
                        <VMenu>
                          ...
                          <template #popper>
                            <div class="p-2 flex-col justify-center items-start gap-2.5 inline-flex">
                              <div class="impala-text-link "
                                   v-on:click="select_temp_var(key,'vygruzka',key2,elem,adres_vygr.id_pogruzka,2,adres_vygr.adres_pogruzki,'adres_pogruzki')">
                              <i
                                    class="iconsax-add-square"></i> Добавить нового грузоотправителя/грузополучателя
                              </div>
                              <div class="impala-text-link impala-text-link:red"
                    ...
                    <div class="impala-row-input">
                      ...
                      <date-picker type="datetime" valueType="format" v-model:value="adres_vygr.date_ts"
                                   format="DD.MM.YYYY H:mm"
                                   @change="update_one_data_pogruzka(elem,adres_vygr.id_pogruzka,2,adres_vygr.date_ts,'date_ts')"
                                   :editable="false"></date-picker>
                    </div>
                    <input hidden="hidden" type="file" :ref="files" v-on:change="handleFilesUpload()"/>
                    ...
                    <div class="impala-row-input">
                      ...
                      <div v-if="adres_vygr.doc_name_FOTO.length > 0"
                           class="impala-box:dashed flex flex-col gap-2 self-stretch p-2">
                        <template v-for="(docInfo,keyCreatedDocsInfo) in adres_vygr.doc_name_FOTO">
                          <div class="flex gap-2">
                            <span class="">{{ docInfo.name_doc }}</span>
                            <span v-if="docInfo.name_doc!==''"
                                  v-on:click="deleteFileInMultGrade(adres_vygr.id_pogruzka,11,keyCreatedDocsInfo,docInfo.id)"
                                  class="impala-text-link impala-text-link:red"> <i class="iconsax-trash"></i></span>
                          </div>
                        </template>
                      </div>
                    ...
                    <div class="impala-row-input">
                      ...
                      <div v-if="adres_vygr.doc_name_DOC.length > 0"
                           class="impala-box:dashed flex flex-col gap-2 self-stretch p-2">
                        <template v-for="(docInfo,keyCreatedDocsInfo) in adres_vygr.doc_name_DOC">
                          <div class="flex gap-2">
                            <span class="">{{ docInfo.name_doc }}</span>
                            <span v-if="docInfo.name_doc!==''"
                                  v-on:click="deleteFileInMultGrade(adres_vygr.id_pogruzka,10,keyCreatedDocsInfo,docInfo.id)"
                                  class="impala-text-link impala-text-link:red"> <i class="iconsax-trash"></i></span>
                          </div>
                        </template>
                      </div>
                    ...
                    <div class="impala-row-input">
                      ...
                      <div v-if="adres_vygr.doc_name_ACT.length > 0"
                           class="impala-box:dashed flex flex-col gap-2 self-stretch p-2">
                        <template v-for="(docInfo,keyCreatedDocsInfo) in adres_vygr.doc_name_ACT">
                          <div class="flex gap-2">
                            <span class="">{{ docInfo.name_doc }}</span>
                            <span v-if="docInfo.name_doc!==''"
                                  v-on:click="deleteFileInMultGrade(adres_vygr.id_pogruzka,12,keyCreatedDocsInfo,docInfo.id)"
                                  class="impala-text-link impala-text-link:red"> <i class="iconsax-trash"></i></span>
                          </div>
                        </template>
                      </div>
            ...
            <div class="grid grid-cols-3 gap-6 w-full border-t border-[#e3eaea] pt-6">
                  ...
                  <div class="grid grid-cols-4 gap-4">
                    <div class="col-span-3 impala-row-input">
                      ...
                      <div class="flex gap-4 items-center">
                        ...
                        <span>
                          <input type="checkbox" :id="'checkbox-'+ key +'-NDS_check'"
                                 @change="update_one_data(elem,'NDS_check')" v-model="elem.NDS_check">
                  ...
                  <div class="impala-row-input w-full" v-for="(sum,key2) in elem.summa_list">
                    ...
                    <div class="flex gap-4 items-center">
                      <div class="col-span-2">
                        <input @keypress="onlyNumber" @input="checkSumma(elem,sum.id_summa,sum.summa,key2)"
                               @blur="update_one_data_summa(elem,sum.id_summa,'summa',sum.summa)"
                               class="impala-input" v-model="sum.summa"/>
                      </div>
                      <div>
                        <date-picker v-model:value="sum.data" valueType="format" type="date"
                                     format="DD.MM.YYYY"
                                     @change="update_one_data_summa(elem,sum.id_summa,'data',sum.data)">
                        </date-picker>
                      </div>
                      <div>
                        <span v-on:click="deleteSumm(sum.id,key, key2)"
                              class="impala-text-link impala-text-link:red" v-tooltip="'Удалить сумму'"><i
              ...
              <div class="self-stretch flex-col justify-between gap-6 flex">
                <div>
                  <div class="flex">
                    <input type="checkbox" :id="'checkbox-'+ key +'-checked2'" @blur="update_one_data(elem,'checked2')"
                           v-model="elem.checked2">
                    <label :for="'checkbox-'+ key +'-checked2'"><span
                        class="impala-label:base">На терминале</span></label>
                  </div>
                  <template v-if="elem.checked2">
                    <div class="impala-row-input w-full">
                      ...
                      <select @blur="update_one_data(elem,'terminal_TS')" class="col border_input grade_adr"
                              v-model="elem.terminal_TS">
                        <option v-for="(elem2) in termList" v-bind:value=elem2.id class="sel_cust">{{
                            elem2.name
                          }}
                        </option>
                      </select>
                ...
                <div class="flex justify-end">
                  <div class="impala-btn impala-btn-small impala-btn-red"
                       v-on:click="deleteTSFromRightColumn(elem,key)">Удалить ТС
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    select_temp_var(key, pogr_vygr, key1, p1, p2, p3, p4, p5) {
      if (pogr_vygr === 'pogruzka' || pogr_vygr === 'vygruzka') {
        this.newModalPogrVygr()
      }
    ...
    chahgeFrontNames() {
      ...
      for (var i = 0; i < this.spisokTShead.length; i++) {
        for (var j = 0; j < this.spisokTShead[i]['adres_pogruzki_TS'].length; j++) {
          findAdresa.push(this.spisokTShead[i]['adres_pogruzki_TS'][j]['adres_pogruzki']);
        }
        for (var j = 0; j < this.spisokTShead[i]['adres_vygr_TS'].length; j++) {
          findAdresa.push(this.spisokTShead[i]['adres_vygr_TS'][j]['adres_pogruzki']);
        }
      }
    ...
    changeFrontNames(gruzColect) {
      console.log('changeFrontNames(gruzColect)');
      ...
      for (var i = 0; i < this.spisokTShead.length; i++) {
        for (var j = 0; j < this.spisokTShead[i]['adres_pogruzki_TS'].length; j++) {
          nazv = this.issetFrontNames(gruzColect, this.spisokTShead[i]['adres_pogruzki_TS'][j]['adres_pogruzki'])
          if (nazv) {
            this.spisokTShead[i]['adres_pogruzki_TS'][j]['adres_pogruzki_show'] = nazv
          } else {
            this.spisokTShead[i]['adres_pogruzki_TS'][j]['adres_pogruzki_show'] = ''
            this.spisokTShead[i]['adres_pogruzki_TS'][j]['adres_pogruzki'] = '' // Если название пустое, очищаем id адреса
          }
        }
        for (var j = 0; j < this.spisokTShead[i]['adres_vygr_TS'].length; j++) {
          nazv = this.issetFrontNames(gruzColect, this.spisokTShead[i]['adres_vygr_TS'][j]['adres_pogruzki'])
          if (nazv) {
            this.spisokTShead[i]['adres_vygr_TS'][j]['adres_vygruzki_show'] = nazv
          } else {
            this.spisokTShead[i]['adres_vygr_TS'][j]['adres_vygruzki_show'] = ''
            this.spisokTShead[i]['adres_vygr_TS'][j]['adres_pogruzki'] = '' // Если название пустое, очищаем id адреса
          }
        }
      }
    },
    ...
    newModalPogrVygr() {
      this.$refs.modalComponentforActionAuthor.newModal()
    },
    changePogrVygrAllShow(id) {
      this.$refs.modalComponentforActionAuthor.setNewModalStatusFalse()
