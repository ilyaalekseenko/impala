...
class PerevozkaFiles extends Model
{
    use HasFactory;

    protected $guarded = false;

    public function saveFile($file,$perevozka_id)
    {
        if($file['doc_path']!='')
        {
            PerevozkaFiles::create([
    ...
    {
        return PerevozkaFiles::where('perevozka_id',$id)->get();
    }
    public function deleteFilesFromDB($id)
    {
        PerevozkaFiles::where('id', $id)->delete();
    }
    ...
    public function updateOldFiles($doc_files,$perevozka_id)
    {
        ...
        if($doc_files)
        {
            ...
            $old_docs=PerevozkaFiles::where('perevozka_id', $perevozka_id)->get();
            ...
            foreach ($old_docs as $old_doc)
            {
                ...
                if($flag==false)
                {
                    try {
                        ...
                        unlink($path_to_del);
                        PerevozkaFiles::where('id', '=',$old_doc['id'])->delete();
                    }
                    catch (\Throwable $e)
                    {
            ...
            foreach ($doc_files as $doc)
            {
                ...
                if($doc['id']!=null)
                {
                    PerevozkaFiles::where('id', '=',$doc['id'])->update([
